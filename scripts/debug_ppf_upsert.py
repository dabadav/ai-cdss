...
def create_test_df(patient_id, protocol_id, ppf_score, contrib_list, other_col='value'):
    ...
    return pd.DataFrame({
        ...
        'PPF_SCORE': [ppf_score],
        'CONTRIB': [contrib_list], # Keep this as a list to test complex type handling
        'OTHER_COL': [other_col]
    })
...
def run_upsert_logic(ppf_contrib_df):
    ...
    try:
        ...
        if PPF_PARQUET_FILEPATH.exists():
            ...
            existing_ppf['PROTOCOL_ID'] = existing_ppf['PROTOCOL_ID'].astype(str) # Assuming Protocol IDs can be mixed (int/str)
            ppf_contrib_df['PROTOCOL_ID'] = ppf_contrib_df['PROTOCOL_ID'].astype(str) # Ensure consistent string type

            logger.debug(f"Existing PPF before merge:\n{existing_ppf.to_string()}")
            ...
            logger.debug(f"Filtered existing data (rows not updated):\n{filtered_existing_ppf.to_string()}")

            # Step 4: Concatenate the filtered existing data with the new/updated data
            updated_ppf = pd.concat([filtered_existing_ppf, ppf_contrib_df], ignore_index=True)

            logger.debug(f"Combined df after upsert:\n{updated_ppf.to_string()}")
            ...
            print(f"Appended/Updated (by rewriting) to existing Parquet file: {PPF_PARQUET_FILEPATH}")

        else:
            # If file doesn't exist, create it normally for the first time
            logger.debug(f"Creating new Parquet file: {PPF_PARQUET_FILEPATH} with initial data:\n{ppf_contrib_df.to_string()}")
            ppf_contrib_df.to_parquet(PPF_PARQUET_FILEPATH, index=False)
            print(f"Created new Parquet file: {PPF_PARQUET_FILEPATH}")

    except Exception as e:
        logger.error(f"Error during Parquet file upsert: {e}", exc_info=True)
...
expected_df_t1 = initial_data.sort_values(by=['PATIENT_ID', 'PROTOCOL_ID']).reset_index(drop=True)
assert_df_equal = pd.testing.assert_frame_equal(read_current_parquet_file(), expected_df_t1)
print(f"Test 1 (Initial Write) passed. Content:\n{read_current_parquet_file().to_string()}")
...
expected_df_t2 = pd.concat([initial_data, new_inserts], ignore_index=True).sort_values(by=['PATIENT_ID', 'PROTOCOL_ID']).reset_index(drop=True)
pd.testing.assert_frame_equal(read_current_parquet_file(), expected_df_t2)
print(f"Test 2 (Pure Insert) passed. Content:\n{read_current_parquet_file().to_string()}")


# Test 3: Pure Update (modify existing rows)
update_data = pd.concat([
    create_test_df(1, 'P1', 0.95, [0.9, 0.9], 'UPDATED_VAL'), # Update existing (1, P1)
], ignore_index=True)
run_upsert_logic(update_data)
# Expected: (1, P1) is updated, (1, P2) and (2, P1) remain as they were after Test 2
expected_df_t3 = pd.concat([
...
], ignore_index=True).sort_values(by=['PATIENT_ID', 'PROTOCOL_ID']).reset_index(drop=True)
pd.testing.assert_frame_equal(read_current_parquet_file(), expected_df_t3)
print(f"Test 3 (Pure Update) passed. Content:\n{read_current_parquet_file().to_string()}")
...
print(f"Test 4 (Mixed Insert/Update) passed. Content:\n{read_current_parquet_file().to_string()}")
...
run_upsert_logic(pd.DataFrame(columns=current_content_before_empty_write.columns)) # Empty DF with correct columns
pd.testing.assert_frame_equal(read_current_parquet_file(), current_content_before_empty_write)
print(f"Test 5 (Empty ppf_contrib_df) passed. Content:\n{read_current_parquet_file().to_string()}")

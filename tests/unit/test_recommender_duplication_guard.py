...
def _make_iface(staging_count: int):
    """Build a CDSS whose store reports an existing prescription iff
    `staging_count > 0`. The store is the write-side port (PrescriptionStore)
    — the orchestrator no longer touches a DB interface directly."""
    fake_store = SimpleNamespace(
        already_prescribed=MagicMock(return_value=staging_count > 0),
        save_prescriptions=MagicMock(),
        save_metrics=MagicMock(),
    )
    fake_repository = SimpleNamespace()
    iface = CDSS.__new__(CDSS)
    iface.repository = fake_repository
    iface.store = fake_store
...
def test_process_patient_skips_when_already_prescribed():
    iface, fake_store = _make_iface(staging_count=35)
    cdss = _fake_cdss()
    ...
    fake_store.save_prescriptions.assert_not_called()

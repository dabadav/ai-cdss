...
class DataPipeline:
    ...
    def process(self, data, scoring_date):
        """
        ...
        """
        session_data, ppf_data = self._prepare_session_data(data, scoring_date)
        weeks_since_start_df = self.feature_builder.build_week_since_start(session_data)
        if not session_data.empty:
            feat_pp_df = self._build_features(
                session_data, ppf_data, weeks_since_start_df, scoring_date
            )
            scoring_input = self._impute_features(feat_pp_df)
        else:
            scoring_input = self._handle_empty_case(ppf_data, weeks_since_start_df)
    ...
    def _prepare_session_data(self, data, scoring_date):
        ...
        return session_data, ppf_data

    def _build_features(
        self, session_data, ppf_data, weeks_since_start_df, scoring_date
    ):
        """
        ...
        """
        dm_df = self.feature_builder.build_delta_dm(
            session_data[BY_PPS + [SESSION_DATE, DM_VALUE]].dropna()
        )
        adherence_df = self.feature_builder.build_recent_adherence(session_data)
        usage_df = self.feature_builder.build_usage(session_data)
        usage_week_df = self.feature_builder.build_week_usage(
            session_data, scoring_date
        )
        days_df = self.feature_builder.build_prescription_days(
            session_data, scoring_date
        )
        feat_pps_df = reduce(
    ...
    def _handle_empty_case(self, ppf_data, weeks_since_start_df):
        """
